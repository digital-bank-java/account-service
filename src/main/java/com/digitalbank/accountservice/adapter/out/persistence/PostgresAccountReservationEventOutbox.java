...
class PostgresAccountReservationEventOutbox implements AccountReservationEventOutbox {
    ...
    PostgresAccountReservationEventOutbox(
            SpringDataAccountReservationEventOutboxRepository repository, ObjectMapper objectMapper) {
    ...
    public boolean recordIfAbsent(AccountReservationEvent event) {
        ...
        try {
            repository.saveAndFlush(new AccountReservationEventOutboxJpaEntity(
                    event, objectMapper.writeValueAsString(event), Instant.now()));
            return true;
