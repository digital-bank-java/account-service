...
public class ReservationEventParser {
    ...
    private static ReservationRequestedEvent requested(JsonNode root, Metadata metadata, String fingerprint) {
        ...
        return new ReservationRequestedEvent(
                ...
                uuid(root, "transactionId"),
                requestId,
                accountId(root, "sourceAccountId"),
                accountId(root, "destinationAccountId"),
                amount(root, "amount"),
    ...
    private static ReservationReleaseRequestedEvent releaseRequested(
            JsonNode root, Metadata metadata, String fingerprint) {
        var aggregateId = text(root, "aggregateId");
        var requestId = text(root, "reservationRequestId");
        if (!aggregateId.equals(requestId)) throw invalid("Reservation aggregate id must equal reservation request id");
        var reason = text(root, "reason");
        if (!java.util.Set.of("TRANSFER_CANCELED", "TRANSFER_TIMEOUT", "MANUAL_COMPENSATION", "LEDGER_POSTING_FAILED")
                .contains(reason)) {
            throw invalid("Unsupported reservation release reason");
        }
        return new ReservationReleaseRequestedEvent(
                metadata.eventId(),
                text(root, "eventType"),
                metadata.schemaVersion(),
                metadata.producer(),
                ...
                uuid(root, "transactionId"),
                requestId,
                uuid(root, "reservationId"),
                accountId(root, "sourceAccountId"),
                reason,
                optionalText(root, "postingRequestId"),
                fingerprint);
    }
    ...
    private static Metadata metadata(JsonNode root, Map<String, String> headers) {
        ...
        if (!"transaction-service".equals(producer)
                || !"1.0.0".equals(schemaVersion)
    ...
    private static AccountId accountId(JsonNode root, String name) {
        return new AccountId(uuid(root, name));
    }

    private static UUID uuid(JsonNode root, String name) {
        return uuidText(text(root, name));
    }
    ...
    private static UUID uuidText(String value) {
        try {
            ...
            throw invalid("Invalid UUID identifier");
        }
    }
    ...
    private static Instant instant(JsonNode root, String name) {
        try {
            ...
            throw invalid("Invalid timestamp: " + name);
        }
    }
    ...
    private static String fingerprint(String payload) {
        try {
            ...
            throw new IllegalStateException("Could not fingerprint reservation event", exception);
        }
    }

    private static InvalidReservationEventException invalid(String message) {
        return new InvalidReservationEventException(message);
    }
