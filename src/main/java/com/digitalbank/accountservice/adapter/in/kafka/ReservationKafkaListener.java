...
class ReservationKafkaListener {
    ...
    ReservationKafkaListener(ReservationEventParser parser, AccountReservationTransportService service) {
        this.parser = parser;
        this.service = service;
    }

    @KafkaListener(
            topics = "${account.reservation.kafka.requested-topic}",
            groupId = "${account.reservation.kafka.group-id}",
            containerFactory = "reservationKafkaListenerContainerFactory")
    void consumeRequested(ConsumerRecord<String, String> record) {
        service.handle((ReservationRequestedEvent)
                parser.parse(record.value(), headers(record), "AccountReservationRequested.v1"));
    }

    @KafkaListener(
            topics = "${account.reservation.kafka.release-requested-topic}",
            groupId = "${account.reservation.kafka.group-id}",
            containerFactory = "reservationKafkaListenerContainerFactory")
    void consumeReleaseRequested(ConsumerRecord<String, String> record) {
        service.handle((ReservationReleaseRequestedEvent)
                parser.parse(record.value(), headers(record), "AccountReservationReleaseRequested.v1"));
    }

    private static Map<String, String> headers(ConsumerRecord<String, String> record) {
        var values = new LinkedHashMap<String, String>();
        record.headers()
                .forEach(header -> values.put(header.key(), new String(header.value(), StandardCharsets.UTF_8)));
