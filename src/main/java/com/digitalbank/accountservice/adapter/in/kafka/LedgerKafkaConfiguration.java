...
public class LedgerKafkaConfiguration {
    ...
    @Bean
    KafkaSecurityBoundary ledgerKafkaSecurityBoundary(Environment environment, LedgerKafkaProperties properties) {
    ...
    DefaultErrorHandler ledgerKafkaErrorHandler(
        ...
        var recoverer = new DeadLetterPublishingRecoverer(
                kafkaTemplate, (record, exception) -> new TopicPartition(record.topic() + ".dlq", -1));
    ...
    static void validateSecurityBoundary(boolean enabled, String securityProtocol, boolean allowInsecureTransport) {
        var insecureTransport =
                "PLAINTEXT".equalsIgnoreCase(securityProtocol) || "SASL_PLAINTEXT".equalsIgnoreCase(securityProtocol);
