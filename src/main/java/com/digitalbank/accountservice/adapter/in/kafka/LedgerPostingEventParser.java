...
public class LedgerPostingEventParser {
    ...
    private static GovernedLedgerPostingEvent.Completed completed(
            JsonNode root, GovernedLedgerPostingEvent.Metadata metadata) {
        ...
        return new GovernedLedgerPostingEvent.Completed(
                ...
                text(root, "reservationRequestId"),
                postingId,
                text(root, "postingRequestId"),
                optionalUuid(root, "reversalOfLedgerEntryId"),
                text(root, "currency"),
                lines(root));
    }
    ...
    private static GovernedLedgerPostingEvent.Failed failed(
        ...
        if (!aggregateId.equals(postingRequestId)
                || !java.util.Set.of("VALIDATION_ERROR", "CONFLICT", "ACCOUNTING_ERROR", "INTERNAL_ERROR")
    ...
    private static GovernedLedgerPostingEvent.Metadata metadata(JsonNode root, Map<String, String> headers) {
        ...
        if (!EXPECTED_PRODUCER.equals(producer)
                || !SUPPORTED_SCHEMA_VERSION.equals(schemaVersion)
        ...
        return new GovernedLedgerPostingEvent.Metadata(
                eventId, correlationId, causationId, producer, schemaVersion, occurredAt);
    }
    ...
    private static List<GovernedLedgerPostingEvent.Line> lines(JsonNode root) {
        ...
        if (values.size() < 2
                ...
                || values.stream()
                        .anyMatch(line -> !"DEBIT".equals(line.lineType()) && !"CREDIT".equals(line.lineType()))) {
