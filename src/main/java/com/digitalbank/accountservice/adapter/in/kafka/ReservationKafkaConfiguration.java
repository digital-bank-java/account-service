...
public class ReservationKafkaConfiguration {
    ...
    @Bean
    ReservationEventParser reservationEventParser() {
        return new ReservationEventParser();
    }
    ...
    KafkaTemplate<String, String> reservationKafkaTemplate(
            @org.springframework.beans.factory.annotation.Qualifier("reservationProducerFactory")
    ...
    DefaultErrorHandler reservationKafkaErrorHandler(
            @org.springframework.beans.factory.annotation.Qualifier("reservationKafkaTemplate")
                    KafkaTemplate<String, String> template,
            ReservationKafkaProperties properties) {
        var recoverer = new DeadLetterPublishingRecoverer(
                template, (record, exception) -> new TopicPartition(record.topic() + ".dlq", -1));
        var handler = new DefaultErrorHandler(
                recoverer,
                new FixedBackOff(properties.getRetryDelayMs(), Math.max(0, properties.getRetryAttempts() - 1L)));
        handler.addNotRetryableExceptions(
                InvalidReservationEventException.class,
                ReservationEventConflictException.class,
    ...
    ConcurrentKafkaListenerContainerFactory<String, String> reservationKafkaListenerContainerFactory(
            @org.springframework.beans.factory.annotation.Qualifier("reservationConsumerFactory")
                    ConsumerFactory<String, String> consumerFactory,
            @org.springframework.beans.factory.annotation.Qualifier("reservationKafkaErrorHandler")
    ...
    static void validateSecurityBoundary(boolean enabled, String protocol, boolean allowInsecure) {
        ...
        if (enabled && insecure && !allowInsecure)
            throw new IllegalStateException(
                    "Enabled reservation Kafka requires authenticated transport; explicitly allow plaintext only for SIT");
    }

    @Bean
    Object reservationKafkaSecurityBoundary(Environment environment, ReservationKafkaProperties properties) {
        var protocol = environment.getProperty(
    ...
    private static Map<String, Object> common(Environment environment) {
        ...
        if (securityProtocol == null)
            securityProtocol = environment.getProperty("spring.kafka.properties.security.protocol");
