...
public class AccountService
    ...
    public Account openAccount(
        ...
        var account = Account.open(
                AccountId.newId(), customerId, accountNumber, iban, type, currency, openingRequestId, clock.instant());
    ...
    public PaginatedAccountProfiles listAccounts(ListAccountsQuery query) {
        var result = accountRepository.search(new AccountSearchCriteria(
                ...
                query.pageSize(),
                query.sort()));
        var items = result.accounts().stream().map(AccountProfile::fromAccount).toList();
