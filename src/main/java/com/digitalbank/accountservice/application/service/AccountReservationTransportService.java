...
public class AccountReservationTransportService {
    ...
    public void recordLedgerFailureRelease(
            ReservationView reservation, UUID eventId, String postingRequestId, Instant occurredAt) {
        var event =
                releasedEvent(reservation, eventId.toString(), "LEDGER_POSTING_FAILED", postingRequestId, occurredAt);
    ...
    private ReservationTransportResult handleRequest(ReservationRequestedEvent event) {
        ...
        var attempt = reservationService.reserveForTransport(
                new com.digitalbank.accountservice.application.port.in.ReserveFundsCommand(
                        ...
                        AccountReservationEventFactory.eventId(
                                "AccountReservationAccepted.v1", event.reservationRequestId())));
        if (attempt.accepted()) {
    ...
    private ReservationTransportResult handleRelease(ReservationReleaseRequestedEvent event) {
        ...
        var now = clock.instant();
        var reservation = reservationService.release(
                event.sourceAccountId(), event.reservationId(), event.reservationRequestId(), now);
        ...
        if (reservation.status() == ReservationStatus.RELEASED) {
            outbox.recordIfAbsent(releasedEvent(
                    reservation, event.eventId().toString(), event.reason(), event.postingRequestId(), now));
        }
        inbox.save(event, now);
    ...
    private ReservationTransportResult replayRelease(
        ...
        return new ReservationTransportResult(
                event.eventId(),
                existing.reservationId() == null ? ReservationStatus.RELEASED : ReservationStatus.RELEASED,
                true);
    }
    ...
    public static AccountReservationEvent releasedEvent(
        ...
        return new AccountReservationEvent(
                AccountReservationEventFactory.eventId(
                        "AccountReservationReleased.v1", reservation.reservationRequestId()),
                "AccountReservationReleased.v1",
                AccountReservationEvent.SCHEMA_VERSION,
                AccountReservationEvent.PRODUCER,
                now,
