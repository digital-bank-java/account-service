...
public class AccountReservationService implements ReserveFundsInputPort {
    ...
    public AccountReservationService(
            AccountRepository accountRepository, AccountReservationRepository reservationRepository, Clock clock) {
    ...
    public ReservationView release(AccountId accountId, UUID reservationId, String reservationRequestId, Instant now) {
        return executeInTransaction(() -> {
            ...
            if (!reservation.reservationId().equals(reservationId)
                    || !reservation.accountId().equals(accountId)) {
    ...
    private static ReservationView replayOrReject(ReservationView existing, ReserveFundsCommand command) {
        if (existing.accountId().equals(command.accountId())
                && (command.destinationAccountId() == null
                        || java.util.Objects.equals(existing.destinationAccountId(), command.destinationAccountId()))
                && (command.transactionId() == null
                        || java.util.Objects.equals(existing.transactionId(), command.transactionId()))
