...
class ApiExceptionHandlerTest {
	...
	void mapsDomainConflictsToStableConflictProblemDetails() {
		var cases = List.of(
				new Case(
						new AccountStatusConflictException(new AccountId(UUID.randomUUID()), AccountStatus.SUSPENDED, "reserve funds"),
						"account-status-conflict", "Account status conflict"),
				new Case(
				...
				new Case(
						...
						"reservation-state-conflict", "Reservation state conflict"),
				new Case(
						new LedgerPostingOutcomeConflictException("posting conflict"),
						"ledger-posting-outcome-conflict", "Ledger posting outcome conflict"),
				new Case(
						new ReservationRequestConflictException("reservation-003"),
						"reservation-request-conflict", "Reservation request conflict"));
		...
		for (var testCase : cases) {
			...
			assertThat(response.getBody()).isNotNull().satisfies(problem -> {
				...
				assertThat(problem.getType()).hasToString("https://digital-bank-java.local/problems/" + testCase.type());
