...
class LedgerPostingKafkaListenerTest {
	...
	void consumesSerializedReversalThroughRealParserAndMapper() {
		...
		var listener = new LedgerPostingKafkaListener(
				...
				new LedgerPostingEventMapper(new FixedReservationRepository(reservation), new FixedAccountRepository(account)),
				outcomePort);

		listener.consumeCompleted(record);

		assertThat(outcomePort.command).isEqualTo(new LedgerPostingOutcomeCommand(
				eventId.toString(),
	...
	private static String reversalPayload(
		...
		return """
				...
				""".formatted(eventId, occurredAt, reversalPostingId, reversalPostingId, originalPostingId, reservedAccountId);
	}
	...
	private static final class CapturingOutcomePort implements LedgerPostingOutcomeInputPort {
		...
		@Override
		public LedgerPostingOutcomeResult handle(
