...
class LedgerKafkaConfigurationSecurityTest {
	...
	void classifiesReservationStateConflictAsNonRetryable() {
		...
		var handler = new LedgerKafkaConfiguration().ledgerKafkaErrorHandler(
				new KafkaTemplate<>(producerFactory), properties);

		assertThat(handler.removeClassification(ReservationStateConflictException.class)).isFalse();
	}
}
