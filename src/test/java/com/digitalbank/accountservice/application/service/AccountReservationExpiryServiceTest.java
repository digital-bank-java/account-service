...
class AccountReservationExpiryServiceTest {
	...
	void expiresDueActiveHoldsAndRestoresAvailabilityAcrossAccountStatuses() {
		...
		assertThat(reservationRepository.findByReservationRequestId("expired-hold").orElseThrow().status())
				.isEqualTo(ReservationStatus.EXPIRED);
		assertThat(accountRepository.findById(suspendedAccount.id()).orElseThrow().availableBalance())
				.isEqualByComparingTo("100.00");
		assertThat(reservationRepository.findByReservationRequestId("future-hold")).contains(future);
