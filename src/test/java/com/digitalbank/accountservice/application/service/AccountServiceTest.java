...
class AccountServiceTest {
    ...
    void opensAccountForCustomer() {
        ...
        var account = accountService.openAccount(
                customerId, "1000000001", "AE070331234567890123456", AccountType.CURRENT, "AED", "open-request-001");
    ...
    void supportsMultipleAccountsForSameCustomerTypeAndCurrency() {
        ...
        var secondAccount = accountService.openAccount(
                customerId, "1000000002", "AE070331234567890123457", AccountType.CURRENT, "AED", "open-request-002");
    ...
    void listsAccountsForAdministration() {
        var customerId = CustomerId.newId();
        var account = accountService.openAccount(
                customerId, "1000000001", "AE070331234567890123456", AccountType.SAVINGS, "USD", "open-request-001");

        var page = accountService.listAccounts(
                new ListAccountsQuery(customerId, AccountStatus.ACTIVE, AccountType.SAVINGS, "USD", 0, 20, List.of()));
    ...
    private static final class InMemoryAccountRepository implements AccountRepository {
        ...
        public AccountSearchResult search(AccountSearchCriteria criteria) {
            var matches = accounts.stream()
                    .filter(account -> criteria.customerId() == null
                            || account.customerId().equals(criteria.customerId()))
                    .filter(account ->
                            criteria.status() == null || account.status().equals(criteria.status()))
                    .filter(account ->
                            criteria.accountType() == null || account.type().equals(criteria.accountType()))
                    .filter(account ->
                            criteria.currency() == null || account.currency().equals(criteria.currency()))
                    .toList();
